...
GitHub Pages does not apply the repository's `_headers` file. MuscleMap 1.4.3 therefore keeps its ORT worker assets under `/musclemap/`, inside the app's isolation service-worker scope. The composite builder still shares ORT with other apps and retains only MuscleMap's checksum-matched local copy. A browser smoke test creates a real ORT session, uses every processor reported by `navigator.hardwareConcurrency`, and completes one inference.

MuscleMap already uses ONNX Runtime Web's multithreaded WebAssembly path. Do not add another proxy worker or assume that an ONNX Runtime upgrade will unlock a new CPU-threading mode. Measure `1`, `2`, `4`, and `8` threads and separate time spent in `session.run()` from the JavaScript pipeline. Keep the requested all-logical-core policy until that benchmark establishes a faster cap.
...
WebGPU is a different execution provider, not a newer WASM thread mode. MuscleMap already imports `ort.webgpu.min.js`, detects `navigator.gpu`, and creates sessions with `['webgpu', 'wasm']`. The fallback lets unsupported nodes run on WASM, where `numThreads` still matters. ORT recommends WebGPU for compute-intensive models and WASM for smaller models or devices without suitable GPU support. [ORT WebGPU guide](https://onnxruntime.ai/docs/tutorials/web/ep-webgpu.html)
