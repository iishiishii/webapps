...
export class StepPipelineState {
  constructor(config = {}) {
    this.nodeOrder = config.nodeOrder || PIPELINE_NODE_ORDER;
    this.stageToNode = config.stageToNode || STAGE_TO_NODE;
    this.nodeToStages = config.nodeToStages || NODE_TO_STAGES;
    this.dependencies = config.dependencies || DEPENDENCIES;
    this.descendants = buildDescendants(this.dependencies);
    this.reset();
  }
  ...
  invalidateFrom(nodeId, { includeSelf = false } = {}) {
    const nodes = includeSelf
      ? [nodeId, ...(this.descendants[nodeId] || [])]
  ...
  computeDependencyHash(nodeId) {
    ...
    const node = this._getNode(nodeId);
    const parentHashes = (this.dependencies[nodeId] || []).map(parentId => {
  ...
  getStageArtifactsInOrder() {
    ...
    for (const nodeId of this.nodeOrder) {
      for (const stage of this.nodeToStages[nodeId] || []) {
  ...
  getNodeForStage(stage) {
    return this.stageToNode[stage] || stage;
  }
